...
class gesture_datasets(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        image_path = 'IBMDvsGesture/3x100ms-Ignore-Overlap/Denoise+Normalize/' + self.imgs[index][0]
...
def load_data_gesture(batch_size, resize=None):
    ...
    gesture_train = gesture_datasets('IBMDvsGesture/3x100ms-Ignore-Overlap/Denoise+Normalize/train/path-label_train.txt', transform=transform)
    t1 = time.time()
    gesture_test = gesture_datasets('IBMDvsGesture/3x100ms-Ignore-Overlap/Denoise+Normalize/test/path-label_test.txt', transform=transform)
    t2 = time.time()
