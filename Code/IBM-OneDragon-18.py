...
def PathLabel(TrainDataPath, TrainTxtPath, TestDataPath, TestTxtPath):
    if __name__ == '__main__':
        ...
        # 接下来创建train和test的txt文件
        train_data = get_files_list(TrainDataPath)
        write_txt(train_data, TrainTxtPath, mode="w")

        test_data = get_files_list(TestDataPath)
        write_txt(test_data, TestTxtPath, mode="w")
...
def load_data_gesture(batch_size, TrainTxtPath, TestTxtPath, resize=None):
    transform = []
    if resize:
        transform.append(torchvision.transforms.Resize(size=resize))
    transform.append(torchvision.transforms.ToTensor())
    transform = torchvision.transforms.Compose(transform)
    gesture_train = gesture_datasets(TrainTxtPath, transform=transform)
    t1 = time.time()
    gesture_test = gesture_datasets(TestTxtPath, transform=transform)
    t2 = time.time()
...
def ConfusionMatrix(net, test_iter, device, FigName):
...
def TrainAndTest(net, train_iter, test_iter, optimizer, device, num_epochs, scheduler=None):
    net = net.to(device)
    ...
    # 混淆矩阵
    ConfusionMatrix(net, test_iter, device, 'Confusion Matrix.jpg')
...
# 生成Path-Label文件
TrainDataPath = 'train'
TrainTxtPath = 'path-label_train.txt'
TestDataPath = 'test'
TestTxtPath = 'path-label_test.txt'
PathLabel(TrainDataPath, TrainTxtPath, TestDataPath, TestTxtPath)

net = torch.nn.DataParallel(net)
batch_size = 250
train_iter, test_iter, t_ImgInDelay = load_data_gesture(batch_size, TrainTxtPath, TestTxtPath)
...
TrainAndTest(net, train_iter, test_iter, optimizer, device, num_epochs, scheduler=scheduler)